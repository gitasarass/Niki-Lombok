...
if(isset($_POST['order'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, number, email, method, checkin, checkout, total_price) VALUES(?,?,?,?,?,?,?,?,?)");
      $insert_order->execute([$user_id, $name, $number, $email, $method, $checkin, $total_products, $checkout, $total_price]);
...
<section class="checkout-orders">
   ...
   <form action="" method="POST">
      ...
      ?>
   <input type="hidden" name="total_price" value="<?= $fetch_cart['price']; ?>">
   <div class="grand-total">grand total : <span>IDR <?= $grand_total; ?>K/MALAM</span></div>
</div>
